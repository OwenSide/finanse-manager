    ...
    <div className="p-4 max-w-3xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <input type="number" placeholder="Kwota" className="p-2 border" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} />
          <select className="p-2 border" value={form.categoryId} onChange={(e) => setForm({ ...form, categoryId: e.target.value })}>
            <option value="">Kategoria</option>
            {categories.map((c) => (
              <option key={c.id} value={c.id}>{c.name}</option>
            ))}
          </select>
          <input type="date" className="p-2 border" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} />
          <select className="p-2 border" value={form.walletId} onChange={(e) => setForm({ ...form, walletId: e.target.value })}>
            <option value="">Portfel</option>
            {predefinedWallets.map((w) => (
              <option key={w.id} value={w.id}>{w.name}</option>
            ))}
          </select>
          <input type="text" placeholder="Komentarz" className="p-2 border col-span-1 sm:col-span-2" value={form.comment} onChange={(e) => setForm({ ...form, comment: e.target.value })} />
        </div>
        <button className="mt-2 px-4 py-2 bg-blue-500 text-white rounded" onClick={handleAddTransaction}>Dodaj</button>
      </div>

      <div className="mb-6">
        <h2 className="font-semibold mb-2">Filtry</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <input type="date" className="p-2 border" placeholder="Od" onChange={(e) => setFilter({ ...filter, dateFrom: e.target.value })} />
          <input type="date" className="p-2 border" placeholder="Do" onChange={(e) => setFilter({ ...filter, dateTo: e.target.value })} />
          <select className="p-2 border" onChange={(e) => setFilter({ ...filter, categoryId: e.target.value })}>
            <option value="">Wszystkie kategorie</option>
            {categories.map((c) => (
              <option key={c.id} value={c.id}>{c.name}</option>
            ))}
          </select>
          <select className="p-2 border" onChange={(e) => setFilter({ ...filter, type: e.target.value })}>
            <option value="">Wszystkie rodzaje</option>
            <option value="income">Przychód</option>
            <option value="expense">Wydatek</option>
          </select>
          <select className="p-2 border col-span-1 sm:col-span-2" onChange={(e) => setFilter({ ...filter, walletId: e.target.value })}>
